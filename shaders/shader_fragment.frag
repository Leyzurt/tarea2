...
void main(void)
{
 ...
 /*  vec3 normalMap = texture(texture_n, Texcoord).rgb;
   normalMap = 2.0 * normalMap - vec3(1.0, 1.0, 1.0);
   vec3 NewNormal = TBN * normalMap;
   ...
   if(u_type_light.x == 1.0f){ //DIRECCIONAL
   ...
   }else if(u_type_light.y == 1.0f){ //PUNTUAL
 ...
 //  finalValue = texture(texture_n, Texcoord).rgb;
...
//shadow /= 9.0;
resultingColor = resultingColor * shadow;
